class Dashing.ProgressBars extends Dashing.Widget
      ...
      progress_items.forEach (item) =>
        ...
        if elem.length
          @animateProgressBarContent(elem[0], parseFloat(elem[0].style.width),
                                    parseFloat(item.progress), 1000, item.value )
        ++counter
  ...
  # /
  animateProgressBarContent: (element, from, to, baseDuration, str) ->
      ...
      interval = setInterval(
        ->
          ...
          if Math.abs(currentValue - from) >= Math.abs(endpointDifference)
            setProgressBarValue(element, to, str)
            clearInterval(interval)
          else
            setProgressBarValue(element, currentValue, str)
        stepInterval)
  ...
  # /
  setProgressBarValue = (element, value, str) ->
    # if (value > 100) 
      # value = 100
    # else if (value < 0) 
      # value = 0
